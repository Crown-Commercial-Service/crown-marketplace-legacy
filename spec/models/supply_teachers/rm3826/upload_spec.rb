...
RSpec.describe SupplyTeachers::RM3826::Upload, type: :model do
  describe 'create' do
    ...
    context 'when supplier list is empty' do
      ...
      it 'does not create any suppliers' do
        ...
        end.not_to change(SupplyTeachers::RM3826::Supplier, :count)
      end
    end
    ...
    context 'when supplier does not exist' do
      ...
      it 'creates supplier' do
        expect do
          described_class.upload!(suppliers)
        end.to change(SupplyTeachers::RM3826::Supplier, :count).by(1)
      end

      it 'assigns ID to supplier' do
        described_class.upload!(suppliers)

        supplier = SupplyTeachers::RM3826::Supplier.last
      ...
      it 'creates a branch associated with supplier' do
        expect do
          described_class.upload!(suppliers)
        end.to change(SupplyTeachers::RM3826::Branch, :count).by(1)
      end
      ...
      context 'and supplier has multiple branches' do
        ...
        it 'creates two branches associated with supplier' do
          expect do
            described_class.upload!(suppliers)
          end.to change(SupplyTeachers::RM3826::Branch, :count).by(2)
        end
    ...
    context 'when suppliers already exist' do
      ...
      let!(:first_supplier) { create(:supply_teachers_rm3826_supplier) }
      let!(:second_supplier) { create(:supply_teachers_rm3826_supplier) }

      it 'destroys all existing suppliers' do
        described_class.upload!(suppliers)

        expect(SupplyTeachers::RM3826::Supplier.find_by(id: first_supplier.id))
          .to be_nil
        expect(SupplyTeachers::RM3826::Supplier.find_by(id: second_supplier.id))
          .to be_nil
      end
