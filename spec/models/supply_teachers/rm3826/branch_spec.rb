...
RSpec.describe SupplyTeachers::RM3826::Branch, type: :model do
  subject(:branch) { build(:supply_teachers_rm3826_branch) }

  let(:model_key) { 'activerecord.errors.models.supply_teachers/rm3826/branch' }
  ...
  describe '.search' do
    context 'when there are branches outside of the search area' do
      let(:supplier) do
        create(:supply_teachers_rm3826_supplier).tap do |s|
          create(:supply_teachers_rm3826_rate, job_type: 'nominated', supplier: s)
        end
      end
      ...
      let(:results) do
        described_class.search(
          ...
          rates: SupplyTeachers::RM3826::Rate.direct_provision.nominated_worker,
          radius: 25
        ).to_a
      end
    ...
    context 'when there are suppliers without nominated worker rates' do
      let!(:branch_with_nominated_worker_rates) do
        supplier = create(:supply_teachers_rm3826_supplier)
        create(:supply_teachers_rm3826_rate, job_type: 'nominated', supplier: supplier)
        create(
          :supply_teachers_rm3826_branch,
          supplier: supplier,
      ...
      let!(:branch_with_master_vendor_nominated_worker_rate) do
        supplier = create(:supply_teachers_rm3826_supplier)
        create(:supply_teachers_rm3826_master_vendor_rate, job_type: 'nominated', supplier: supplier)
        create(
          :supply_teachers_rm3826_branch,
          supplier: supplier,
          location: Geocoding.point(latitude: 0, longitude: 0)
    ...
    context 'when there are suppliers with fixed term rates' do
      let!(:branch_with_fixed_term_rates) do
        supplier = create(:supply_teachers_rm3826_supplier)
        create(:supply_teachers_rm3826_rate, job_type: 'fixed_term', supplier: supplier)
        create(
          :supply_teachers_rm3826_branch,
          supplier: supplier,
          location: Geocoding.point(latitude: 0, longitude: 0)
      ...
      let!(:branch_with_master_vendor_fixed_term_rate) do
        supplier = create(:supply_teachers_rm3826_supplier)
        create(:supply_teachers_rm3826_master_vendor_rate, job_type: 'fixed_term', supplier: supplier)
        create(
          :supply_teachers_rm3826_branch,
          supplier: supplier,
          location: Geocoding.point(latitude: 0, longitude: 0)
      ...
      let(:results) do
        ...
        described_class.search(
          point,
          rates: SupplyTeachers::RM3826::Rate.direct_provision.fixed_term,
          radius: 25
        ).to_a
      end
    ...
    context 'when there are suppliers with different nominated worker rates' do
      let!(:branch_of_cheaper_supplier) do
        supplier = create(:supply_teachers_rm3826_supplier)
        create(:supply_teachers_rm3826_rate, job_type: 'nominated', supplier: supplier, mark_up: 0.1)
        create(:supply_teachers_rm3826_branch, supplier: supplier, location: Geocoding.point(latitude: 0, longitude: 0))
      end
      let!(:branch_of_costlier_supplier) do
        supplier = create(:supply_teachers_rm3826_supplier)
        create(:supply_teachers_rm3826_rate, job_type: 'nominated', supplier: supplier, mark_up: 0.9)
        create(:supply_teachers_rm3826_branch, supplier: supplier, location: Geocoding.point(latitude: 0, longitude: 0))
      end
    ...
    context 'when there are suppliers with different rates in different locations' do
      let!(:branch_of_cheapest_supplier) do
        supplier = create(:supply_teachers_rm3826_supplier)
        create(:supply_teachers_rm3826_rate, job_type: 'nominated', supplier: supplier, mark_up: 0.0)
        location = Geocoding.point(latitude: 0.2, longitude: 0.2)
        create(:supply_teachers_rm3826_branch, name: 'cheapest', supplier: supplier, location: location)
      end
      let!(:branch_of_farthest_supplier) do
        supplier = create(:supply_teachers_rm3826_supplier)
        create(:supply_teachers_rm3826_rate, job_type: 'nominated', supplier: supplier, mark_up: 0.1)
        location = Geocoding.point(latitude: 0.1, longitude: 0.1)
        create(:supply_teachers_rm3826_branch, name: 'farthest', supplier: supplier, location: location)
      end
      let!(:branch_of_closest_supplier) do
        supplier = create(:supply_teachers_rm3826_supplier)
        create(:supply_teachers_rm3826_rate, job_type: 'nominated', supplier: supplier, mark_up: 0.1)
        location = Geocoding.point(latitude: 0, longitude: 0)
        create(:supply_teachers_rm3826_branch, name: 'closest', supplier: supplier, location: location)
      end
  ...
  shared_context 'and I have suppliers and branches' do
    before do
      ('a'..'z').each do |supplier_name|
        supplier = create(:supply_teachers_rm3826_supplier, name: supplier_name * 3)

        create(:supply_teachers_rm3826_branch, slug: "branch-#{supplier_name}a", supplier: supplier)
        create(:supply_teachers_rm3826_branch, slug: "branch-#{supplier_name}b", supplier: supplier)
      end
    end
  end
