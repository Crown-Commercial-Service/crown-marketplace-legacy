...
RSpec.describe SupplyTeachers::RM6376::Admin::Upload do
    ...
    context 'when considering if the files are attached' do
      context 'and no files are attached' do
        it 'is not valid and has the correct error messages' do
          ...
          expect(upload.errors.messages.values.flatten).to match ["Select the 'Supplier details' file", "Select the 'Supplier geographical data' file", "Select the 'Supplier rate cards' file"]
        end
      end

      context 'and two files are attached' do
        before do
          upload.supplier_geographical_data_file = valid_file_path
          upload.supplier_rate_cards_file = valid_file_path
        end

        it 'is not valid and has the correct error messages' do
          expect(upload.valid?(:upload)).to be false
          expect(upload.errors.messages.values.flatten).to match ["Select the 'Supplier details' file"]
        end
      end
    end
    ...
    context 'when considering the file extension' do
      context 'and no files have the correct file extension' do
        before do
          ...
          upload.supplier_geographical_data_file = text_file_path
          upload.supplier_rate_cards_file = text_file_path
        end

        it 'is not valid and has the correct error messages' do
          expect(upload.valid?(:upload)).to be false
          expect(upload.errors.messages.values.flatten).to match ["The 'Supplier details' file must be an XLSX", "The 'Supplier geographical data' file must be an XLSX", "The 'Supplier rate cards' file must be an XLSX"]
        end
      end
      ...
      context 'and two files have the correct file extension' do
        ...
        it 'is not valid and has the correct error messages' do
          expect(upload.valid?(:upload)).to be false
          expect(upload.errors.messages.values.flatten).to match ["The 'Supplier geographical data' file must be an XLSX"]
        end
      end
    end
    ...
    context 'when considering the file content type' do
      context 'and no files have the correct content type' do
        before do
          ...
          upload.supplier_geographical_data_file = invalid_file_path
          upload.supplier_rate_cards_file = invalid_file_path
        end

        it 'is not valid and has the correct error messages' do
          expect(upload.valid?(:upload)).to be false
          expect(upload.errors.messages.values.flatten).to match ["The 'Supplier details' file does not contain the expected content type", "The 'Supplier geographical data' file does not contain the expected content type", "The 'Supplier rate cards' file does not contain the expected content type"]
        end
      end
      ...
      context 'and three files have the correct content type' do
        ...
        it 'is not valid and has the correct error messages' do
          expect(upload.valid?(:upload)).to be false
          expect(upload.errors.messages.values.flatten).to match ["The 'Supplier details' file does not contain the expected content type", "The 'Supplier rate cards' file does not contain the expected content type"]
        end
      end
    end
  ...
  describe 'aasm_state' do
    ...
    context 'when start_upload is called' do
      ...
      it 'starts the worker' do
        expect(SupplyTeachers::RM6376::Admin::FileUploadWorker).to have_received(:perform_async).with(upload.id)
      end
    end
