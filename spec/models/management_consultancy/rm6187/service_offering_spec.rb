...
RSpec.describe ManagementConsultancy::RM6187::ServiceOffering do
  ...
  it 'is not valid if offering exists for same lot_number, service_code & supplier' do
    ...
    new_offering = build(
      :management_consultancy_rm6187_service_offering,
  ...
  it 'is valid even if offering exists for same lot_number & service_code, but different supplier' do
    ...
    new_offering = build(
      :management_consultancy_rm6187_service_offering,
      supplier: build(:management_consultancy_rm6187_supplier),
      lot_number: service_offering.lot_number,
      service_code: service_offering.service_code
    )
  ...
  it 'looks up service based on its code' do
    offering = build(:management_consultancy_rm6187_service_offering, service_code: 'MCF3.3.3')
    expect(offering.service).to be_instance_of(ManagementConsultancy::RM6187::Service)
    expect(offering.service.code).to eq('MCF3.3.3')
  end
end
