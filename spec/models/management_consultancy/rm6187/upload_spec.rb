  ...
  describe 'create' do
    ...
    context 'when supplier list is empty' do
      ...
      it 'does not create any suppliers' do
        ...
        end.not_to change(ManagementConsultancy::RM6187::Supplier, :count)
      end
    end
    ...
    context 'when supplier does not exist' do
      ...
      it 'creates supplier' do
        expect do
          described_class.upload!(suppliers)
        end.to change(ManagementConsultancy::RM6187::Supplier, :count).by(1)
      end

      it 'assigns ID to supplier' do
        described_class.upload!(suppliers)

        supplier = ManagementConsultancy::RM6187::Supplier.last
      ...
      context 'and supplier has lots with services' do
        ...
        let(:supplier) { ManagementConsultancy::RM6187::Supplier.last }
        ...
        it 'creates service offerings associated with supplier' do
          expect do
            described_class.upload!(suppliers)
          end.to change(ManagementConsultancy::RM6187::ServiceOffering, :count).by(3)
        end
    ...
    context 'when suppliers already exist' do
      let!(:first_supplier) { create(:management_consultancy_rm6187_supplier) }
      let!(:second_supplier) { create(:management_consultancy_rm6187_supplier) }

      it 'destroys all existing suppliers' do
        described_class.upload!(suppliers)

        expect(ManagementConsultancy::RM6187::Supplier.find_by(id: first_supplier.id))
          .to be_nil
        expect(ManagementConsultancy::RM6187::Supplier.find_by(id: second_supplier.id))
          .to be_nil
      end
