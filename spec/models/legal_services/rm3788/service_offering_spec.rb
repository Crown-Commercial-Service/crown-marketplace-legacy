...
RSpec.describe LegalServices::RM3788::ServiceOffering, type: :model do
  subject(:service_offering) { build(:legal_services_rm3788_service_offering) }
  ...
  it 'is not valid if offering exists for same lot_number, service_code & supplier' do
    ...
    new_offering = build(
      :legal_services_rm3788_service_offering,
  ...
  it 'is valid even if offering exists for same lot_number & service_code, but different supplier' do
    ...
    new_offering = build(
      :legal_services_rm3788_service_offering,
      supplier: build(:legal_services_rm3788_supplier),
      lot_number: service_offering.lot_number,
      service_code: service_offering.service_code
    )
  ...
  it 'looks up service based on its code' do
    offering = build(:legal_services_rm3788_service_offering, service_code: 'WPSLS.1.8')
    expect(offering.service).to be_instance_of(LegalServices::RM3788::Service)
