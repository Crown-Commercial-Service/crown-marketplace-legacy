...
RSpec.describe LegalServices::RM3788::Upload, type: :model do
  describe 'create' do
    ...
    context 'when supplier list is empty' do
      ...
      it 'does not create any suppliers' do
        ...
        end.not_to change(LegalServices::RM3788::Supplier, :count)
      end
    end
    ...
    context 'when supplier does not exist' do
      ...
      it 'creates supplier' do
        expect do
          described_class.upload!(suppliers)
        end.to change(LegalServices::RM3788::Supplier, :count).by(1)
      end

      it 'assigns ID to supplier' do
        described_class.upload!(suppliers)

        supplier = LegalServices::RM3788::Supplier.last
      ...
      context 'and supplier is offering lot 1 services in regions' do
        ...
        let(:supplier) { LegalServices::RM3788::Supplier.last }
        ...
        it 'creates regional availabilities associated with supplier' do
          expect do
            described_class.upload!(suppliers)
          end.to change(LegalServices::RM3788::RegionalAvailability, :count).by(3)
        end
      ...
      context 'and supplier has lots with services' do
        ...
        it 'creates service offerings associated with supplier' do
          expect do
            described_class.upload!(suppliers)
          end.to change(LegalServices::RM3788::ServiceOffering, :count).by(3)
        end
    ...
    context 'when suppliers already exist' do
      let!(:first_supplier) { create(:legal_services_rm3788_supplier) }
      let!(:second_supplier) { create(:legal_services_rm3788_supplier) }

      it 'destroys all existing suppliers' do
        described_class.upload!(suppliers)

        expect(LegalServices::RM3788::Supplier.find_by(id: first_supplier.id))
          .to be_nil
        expect(LegalServices::RM3788::Supplier.find_by(id: second_supplier.id))
          .to be_nil
      end
