...
RSpec.describe LegalServices::RM6374::Journey::InformationAboutYourRequirement do
  ...
  describe 'validations' do
    ...
    context 'when no requirement_being_awarded is provided' do
      ...
      it 'is not valid and has the correct error message' do
        expect(step).not_to be_valid
        expect(step.errors[:requirement_being_awarded].first).to eq 'You must select an option for the probability of awarding through a GCA framework'
      end
    end
  ...
  describe '.next_step_class' do
    it 'returns Journey::ChooseSector' do
      expect(step.next_step_class).to be LegalServices::RM6374::Journey::ChooseSector
    end
  end
