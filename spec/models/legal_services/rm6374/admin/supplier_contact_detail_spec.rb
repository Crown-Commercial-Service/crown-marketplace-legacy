...
RSpec.describe LegalServices::RM6374::Admin::SupplierContactDetail do
  let(:contact_detail) { create(:legal_services_rm6374_admin_supplier_contact_detail) }
  ...
  describe 'validations' do
    ...
    context 'when validating additional_supplier_information' do
      let(:attributes) { { address:, description:, lot_1a_prospectus_link:, lot_1b_prospectus_link:, lot_1c_prospectus_link:, lot_2_prospectus_link:, lot_3_prospectus_link:, lot_4_prospectus_link:, lot_5_prospectus_link:, lot_6_prospectus_link: } }

      let(:address) { 'B.L.A.D.E Barracks, NLA' }
      let(:description) { 'The ultimate B.L.A.D.E' }
      let(:lot_1a_prospectus_link) { Faker::Internet.url }
      let(:lot_1b_prospectus_link) { Faker::Internet.url }
      let(:lot_1c_prospectus_link) { Faker::Internet.url }
      let(:lot_2_prospectus_link) { Faker::Internet.url }
      let(:lot_3_prospectus_link) { nil }
      let(:lot_4_prospectus_link) { nil }
      let(:lot_5_prospectus_link) { nil }
      let(:lot_6_prospectus_link) { Faker::Internet.url }
      ...
      context 'when considering the description' do
        context 'and it is nil' do
          let(:description) { nil }
        ...
        context 'and it is too long' do
          ...
          it 'is invalid and has the correct error message' do
            expect(contact_detail.valid?(:additional_supplier_information)).to be(false)
            expect(contact_detail.errors[:description].first).to eq('The description should be no more than 255 characters')
          end
        end
        ...
        context 'and it is present' do
          context 'and it has spaces' do
            ...
            it 'is valid and removes the excess white space' do
              expect(contact_detail).to be_valid(:additional_supplier_information)
              expect(contact_detail.description).to eq('the description')
            end
          end
      ...
      context 'when considering the lot 1a prospectus link' do
        context 'and it is too long' do
          let(:lot_1a_prospectus_link) { "https://g#{'o' * 255}le.com" }

          it 'is invalid and has the correct error message' do
            expect(contact_detail.valid?(:additional_supplier_information)).to be(false)
            expect(contact_detail.errors[:lot_1a_prospectus_link].first).to eq('The lot 1a prospectus link must be no more than 255 characters')
          end
        end

        context 'and it is not a url' do
          let(:lot_1a_prospectus_link) { 'I am not a url' }

          it 'is invalid and has the correct error message' do
            expect(contact_detail.valid?(:additional_supplier_information)).to be(false)
            expect(contact_detail.errors[:lot_1a_prospectus_link].first).to eq('Enter the lot 1a prospectus link in the correct format, like https://example.com')
          end
        end

        context 'and it is present' do
          context 'and it is nil' do
            let(:lot_1a_prospectus_link) { nil }
          ...
          context 'and it has spaces' do
            let(:lot_1a_prospectus_link) { '   https://example.com   ' }

            it 'is valid and removes the excess white space' do
              expect(contact_detail).to be_valid(:additional_supplier_information)
              expect(contact_detail.lot_1a_prospectus_link).to eq('https://example.com')
            end
          end
      ...
      context 'when considering the lot 1b prospectus link' do
        context 'and it is too long' do
          let(:lot_1b_prospectus_link) { "https://g#{'o' * 255}le.com" }

          it 'is invalid and has the correct error message' do
            expect(contact_detail.valid?(:additional_supplier_information)).to be(false)
            expect(contact_detail.errors[:lot_1b_prospectus_link].first).to eq('The lot 1b prospectus link must be no more than 255 characters')
          end
        end

        context 'and it is not a url' do
          let(:lot_1b_prospectus_link) { 'I am not a url' }

          it 'is invalid and has the correct error message' do
            expect(contact_detail.valid?(:additional_supplier_information)).to be(false)
            expect(contact_detail.errors[:lot_1b_prospectus_link].first).to eq('Enter the lot 1b prospectus link in the correct format, like https://example.com')
          end
        end

        context 'and it is present' do
          context 'and it is nil' do
            let(:lot_1b_prospectus_link) { nil }
          ...
          context 'and it has spaces' do
            let(:lot_1b_prospectus_link) { '   https://example.com   ' }

            it 'is valid and removes the excess white space' do
              expect(contact_detail).to be_valid(:additional_supplier_information)
              expect(contact_detail.lot_1b_prospectus_link).to eq('https://example.com')
            end
          end
      ...
      context 'when considering the lot 1c prospectus link' do
        context 'and it is too long' do
          let(:lot_1c_prospectus_link) { "https://g#{'o' * 255}le.com" }

          it 'is invalid and has the correct error message' do
            expect(contact_detail.valid?(:additional_supplier_information)).to be(false)
            expect(contact_detail.errors[:lot_1c_prospectus_link].first).to eq('The lot 1c prospectus link must be no more than 255 characters')
          end
        end

        context 'and it is not a url' do
          let(:lot_1c_prospectus_link) { 'I am not a url' }

          it 'is invalid and has the correct error message' do
            expect(contact_detail.valid?(:additional_supplier_information)).to be(false)
            expect(contact_detail.errors[:lot_1c_prospectus_link].first).to eq('Enter the lot 1c prospectus link in the correct format, like https://example.com')
          end
        end

        context 'and it is present' do
          context 'and it is nil' do
            let(:lot_1c_prospectus_link) { nil }
          ...
          context 'and it has spaces' do
            let(:lot_1c_prospectus_link) { '   https://example.com   ' }

            it 'is valid and removes the excess white space' do
              expect(contact_detail).to be_valid(:additional_supplier_information)
              expect(contact_detail.lot_1c_prospectus_link).to eq('https://example.com')
            end
          end
      ...
      context 'when considering the lot 6 prospectus link' do
        context 'and it is too long' do
          let(:lot_6_prospectus_link) { "https://g#{'o' * 255}le.com" }

          it 'is invalid and has the correct error message' do
            expect(contact_detail.valid?(:additional_supplier_information)).to be(false)
            expect(contact_detail.errors[:lot_6_prospectus_link].first).to eq('The lot 6 prospectus link must be no more than 255 characters')
          end
        end

        context 'and it is not a url' do
          let(:lot_6_prospectus_link) { 'I am not a url' }

          it 'is invalid and has the correct error message' do
            expect(contact_detail.valid?(:additional_supplier_information)).to be(false)
            expect(contact_detail.errors[:lot_6_prospectus_link].first).to eq('Enter the lot 6 prospectus link in the correct format, like https://example.com')
          end
        end

        context 'and it is present' do
          context 'and it is nil' do
            let(:lot_6_prospectus_link) { nil }
          ...
          context 'and it has spaces' do
            let(:lot_6_prospectus_link) { '   https://example.com   ' }

            it 'is valid and removes the excess white space' do
              expect(contact_detail).to be_valid(:additional_supplier_information)
              expect(contact_detail.lot_6_prospectus_link).to eq('https://example.com')
            end
          end
