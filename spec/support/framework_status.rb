...
RSpec.shared_context 'and RM6187 is live' do
  before { Framework.find_by(framework: 'RM6187').update(expires_at: 1.day.from_now) }
end
...
RSpec.shared_context 'and RM6309 is live in the future' do
  before { Framework.find_by(framework: 'RM6309').update(live_at: 1.day.from_now) }
end

RSpec.shared_context 'and RM6309 is live today' do
  before { Framework.find_by(framework: 'RM6309').update(live_at: Time.zone.now) }
end

RSpec.shared_context 'and RM6309 has expired' do
  before { Framework.find_by(framework: 'RM6309').update(expires_at: Time.zone.now) }
end
