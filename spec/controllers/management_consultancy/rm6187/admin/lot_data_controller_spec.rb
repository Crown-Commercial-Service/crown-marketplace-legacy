...
RSpec.describe ManagementConsultancy::RM6187::Admin::LotDataController do
  ...
  let(:supplier_framework_lot_jurisdiction) { create(:supplier_framework_lot_jurisdiction, supplier_framework_lot: supplier_framework_lot, jurisdiction_id: 'RM6187.GB') }
  let(:lot_number) { '1' }
  ...
  describe 'POST update' do
    ...
    context 'when the lot number is 1' do
      ...
      context 'and the section is rates' do
        let(:section) { 'rates' }
        let(:rates) { Position.where(lot_id: "RM6187.#{lot_number}").pluck(:id).index_with { '2345.67' } }
        ...
        context 'when it is valid' do
          ...
          it 'updates the details' do
            updated_supplier_framework_lot_rates = supplier_framework_lot.rates.where(position_id: Position.where(lot_id: "RM6187.#{lot_number}").select(:id), supplier_framework_lot_jurisdiction_id: supplier_framework_lot_jurisdiction.id)
          ...
          it 'creates a change log' do
            ...
            expect(change_log.change_data['id']).to eq(supplier_framework_lot.id)
            expect(change_log.change_data['jurisdiction_id']).to eq('RM6187.GB')
        ...
        context 'when it is invalid' do
          let(:rates) { Position.where(lot_id: "RM6187.#{lot_number}").pluck(:id).index_with { '' } }
