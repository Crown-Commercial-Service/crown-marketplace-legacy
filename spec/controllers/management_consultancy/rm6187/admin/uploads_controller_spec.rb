  ...
  describe 'POST create' do
    ...
    context 'when the upload is valid' do
      ...
      it 'redirects to the show page' do
        expect(response).to redirect_to management_consultancy_rm6187_admin_upload_path(upload)
      end
  ...
  describe 'GET show' do
    ...
    context 'when the upload has failed' do
      ...
      it 'renders the show template and the failed partial' do
        expect(response).to render_template(:show)
        expect(response).to render_template(partial: 'management_consultancy/rm6187/admin/uploads/_failed')
      end
    end
  end

  describe 'GET progress' do
    let(:upload) { create(:management_consultancy_rm6187_admin_upload, aasm_state: 'publishing_data') }
