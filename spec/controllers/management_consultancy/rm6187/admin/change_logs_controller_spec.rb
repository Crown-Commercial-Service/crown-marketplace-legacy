...
RSpec.describe ManagementConsultancy::RM6187::Admin::ChangeLogsController do
  let(:framework_id) { 'RM6187' }
  let(:default_params) { { service: 'management_consultancy/admin', framework: framework_id } }
  ...
  describe 'GET index CSV' do
    ...
    it 'download the csv' do
      expect(response.headers['Content-Disposition']).to include 'filename="Change log - Management Consultancy - RM6187 - '
