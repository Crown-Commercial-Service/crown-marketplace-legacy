...
RSpec.describe SupplyTeachers::RM6376::Admin::LotDataController do
  let(:default_params) { { service: 'supply_teachers/admin', framework: 'RM6376', supplier_id: supplier_framework.id } }

  let(:supplier_framework) { create(:supplier_framework, framework_id: 'RM6376') }
  let(:supplier_framework_lot) { create(:supplier_framework_lot, supplier_framework: supplier_framework, lot_id: "RM6376.#{lot_number}") }
  let(:supplier_framework_lot_rates) { Position.where(lot_id: "RM6376.#{lot_number}").pluck(:id).map { |position_id| create(:supplier_framework_lot_rate, supplier_framework_lot: supplier_framework_lot, jurisdiction: supplier_framework_lot_jurisdiction, position_id: position_id) } }
  let(:supplier_framework_lot_jurisdiction) { create(:supplier_framework_lot_jurisdiction, supplier_framework_lot: supplier_framework_lot, jurisdiction_id: 'RM6376.GB') }
  ...
  describe 'GET index' do
    ...
    context 'when logged in as an admin' do
      ...
      it 'assigns framework' do
        expect(assigns(:framework).id).to eq('RM6376')
      end
      ...
      it 'assigns supplier_lot_data' do
        expect(assigns(:supplier_lot_data)).to eq(
          [
            ...
            {
              lot: { number: '2', name: 'Managed service provider', number_as_slug: '2' },
              enabled: nil,
              sections: %i[rates]
            },
          ]
  ...
  describe 'GET show' do
    ...
    shared_examples 'when testing a section' do
      ...
      it 'assigns lot' do
        expect(assigns(:lot).id).to eq("RM6376.#{lot_number}")
      end
      ...
      context 'when considering the templates' do
        ...
        it 'renders section partial template' do
          ...
          expect(response).to render_template(partial: "supply_teachers/rm6376/admin/lot_data/show/_#{section}")
        end
      end
    end
    ...
    context 'when the lot number is 1' do
      ...
      context 'when the section is unexpected' do
        ...
        it 'redirects to the index page' do
          expect(response).to redirect_to(supply_teachers_rm6376_admin_supplier_lot_data_path)
        end
      end
    end
  end
  ...
  describe 'POST update' do
    ...
    shared_examples 'when testing a section' do
      ...
      it 'assigns lot' do
        expect(assigns(:lot).id).to eq('RM6376.1')
      end
    ...
    context 'when the lot number is 1' do
      ...
      context 'and the section is rates' do
        let(:section) { 'rates' }
        let(:rates) { Position.where(lot_id: "RM6376.#{lot_number}").pluck(:id, :rate_type).to_h { |position_id, rate_type| [position_id, rate_type == 'percentage' ? '44.5' : '2345.67'] } }
        ...
        context 'when it is valid' do
          ...
          it 'updates the details' do
            updated_supplier_framework_lot_rates = supplier_framework_lot.rates.where(position_id: Position.where(lot_id: "RM6376.#{lot_number}").select(:id), supplier_framework_lot_jurisdiction_id: supplier_framework_lot_jurisdiction.id)
          ...
          it 'creates a change log' do
            ...
            expect(change_log.change_data['id']).to eq(supplier_framework_lot.id)
            expect(change_log.change_data['jurisdiction_id']).to eq('RM6376.GB')
            expect(change_log.change_data['rates'].map { |rate_change| { position_id: rate_change['position_id'], before_rate: rate_change['before'].present?, after: rate_change['after'] } }).to eq(
              [
                { after: 234567, before_rate: true, position_id: 'RM6376.1.1' },
                { after: 234567, before_rate: true, position_id: 'RM6376.1.2' },
                { after: 234567, before_rate: true, position_id: 'RM6376.1.3' },
                { after: 234567, before_rate: true, position_id: 'RM6376.1.4' },
                { after: 234567, before_rate: true, position_id: 'RM6376.1.5' },
                { after: 234567, before_rate: true, position_id: 'RM6376.1.6' },
                { after: 234567, before_rate: true, position_id: 'RM6376.1.7' },
                { after: 234567, before_rate: true, position_id: 'RM6376.1.8' },
                { after: 4450, before_rate: true, position_id: 'RM6376.1.9' },
                { after: 234567, before_rate: true, position_id: 'RM6376.1.10' },
                { after: 4450, before_rate: true, position_id: 'RM6376.1.11' }
              ]
            )
          end
        ...
        context 'when it is invalid' do
          let(:rates) { Position.where(lot_id: "RM6376.#{lot_number}").pluck(:id).index_with { '' } }
