...
RSpec.describe SupplyTeachers::RM6376::Admin::ChangeLogsController do
  let(:framework_id) { 'RM6376' }
  let(:default_params) { { service: 'supply_teachers/admin', framework: framework_id } }
  ...
  describe 'GET index CSV' do
    ...
    it 'download the csv' do
      expect(response.headers['Content-Disposition']).to include 'filename="Change log - Supply Teachers - RM6376 - '
  ...
  describe 'GET show' do
    ...
    context 'when the change type is update_supplier_framework_lot_branch' do
