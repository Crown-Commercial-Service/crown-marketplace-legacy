...
RSpec.describe SupplyTeachers::RM6238::Admin::LotDataController do
  ...
  let(:supplier_framework_lot_jurisdiction) { create(:supplier_framework_lot_jurisdiction, supplier_framework_lot: supplier_framework_lot, jurisdiction_id: 'RM6238.GB') }
      ...
      context 'and the section is rates' do
        ...
        context 'when it is valid' do
          ...
          it 'creates a change log' do
            ...
            expect(change_log.change_data['id']).to eq(supplier_framework_lot.id)
            expect(change_log.change_data['jurisdiction_id']).to eq('RM6238.GB')
