...
RSpec.describe SupplyTeachers::RM3826::SuppliersController, type: :controller do
  ...
  describe 'GET all suppliers' do
    let!(:branch_1) { create(:supply_teachers_rm3826_branch) }
    let!(:branch_2) { create(:supply_teachers_rm3826_branch) }
    ...
    it 'assigns paginated_suppliers' do
      expect(assigns(:paginated_suppliers).map do |supplier|
        {
          supplier_id: supplier.supply_teachers_rm3826_supplier_id,
          name: supplier.name
        }
      end).to include(*[branch_1, branch_2].map do |supplier|
        {
          supplier_id: supplier.supply_teachers_rm3826_supplier_id,
  ...
  describe 'GET search_all_suppliers' do
    let(:supplier_1) { create(:supply_teachers_rm3826_supplier, name: 'aaa') }
    let(:supplier_2) { create(:supply_teachers_rm3826_supplier, name: 'zzz') }
    let(:paginated_supplier_names) { assigns(:paginated_suppliers).map(&:name) }

    before do
      create(:supply_teachers_rm3826_branch, slug: 'branch-a', supplier: supplier_1)
      create(:supply_teachers_rm3826_branch, slug: 'branch-z', supplier: supplier_2)
