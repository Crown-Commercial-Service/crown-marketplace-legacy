...
RSpec.describe ActiveStorage::BlobsController, type: :controller do
  ...
  describe '#show' do
    context 'when trying to download a supply teachers document' do
      ...
      let(:object_id_key) { :st_upload_id }
    ...
    context 'when trying to download a management consultancy document' do
      ...
      let(:object_id_key) { :mc_upload_id }
      let(:object_id) { admin_upload.id }
    ...
    context 'when trying to download a legal services document' do
      let(:signed_id) { admin_upload.supplier_details_file.blob.signed_id }
      let(:filename) { admin_upload.supplier_details_file.blob.filename }
      let(:object_id_key) { :ls_upload_id }
      let(:object_id) { admin_upload.id }
      let(:admin_upload) { create(:legal_services_rm3788_admin_upload_with_document) }
