...
RSpec.describe LegalServices::Admin::HomeController do
  let(:default_params) { { service: 'legal_services/admin' } }

  describe 'GET framework' do
    context 'when RM6374 is live' do
      it 'redirects to the RM6374 admin home page' do
        get :framework
        expect(response).to redirect_to legal_services_admin_index_path('RM6374')
      end
    end
  end
