...
RSpec.describe LegalServices::Admin::UploadsController, type: :controller do
  ...
  describe 'POST create' do
    ...
    before do
      ...
      allow(LegalServices::Admin::Upload).to receive(:new).with(anything).and_return(upload)
      allow(LegalServices::FileUploadWorker).to receive(:perform_async).with(upload.id).and_return(true)
      post :create, params: { legal_services_admin_upload: { supplier_details_file: fake_file, supplier_rate_cards_file: fake_file, supplier_lot_1_service_offerings_file: fake_file, supplier_lot_2_service_offerings_file: fake_file, supplier_lot_3_service_offerings_file: fake_file, supplier_lot_4_service_offerings_file: fake_file } }
    end
    ...
    context 'when the upload is valid' do
      ...
      it 'redirects to the show page' do
        expect(response).to redirect_to legal_services_admin_upload_path(upload)
      end
  ...
  describe 'GET show' do
    let(:upload) { create(:legal_services_admin_upload, aasm_state: aasm_state) }
  ...
  describe 'GET progress' do
    let(:upload) { create(:legal_services_admin_upload, aasm_state: 'publishing_data') }
