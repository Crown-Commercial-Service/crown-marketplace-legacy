...
RSpec.describe LegalPanelForGovernment::HomeController do
  let(:default_params) { { service: 'legal_panel_for_government' } }

  describe 'GET framework' do
    context 'when RM6360 is live' do
      it 'redirects to the RM6360 home page' do
        get :framework
        expect(response).to redirect_to legal_panel_for_government_index_path('RM6360')
      end
    end
  end
