...
RSpec.describe LegalPanelForGovernment::RM6360::Admin::ChangeLogsController do
  let(:framework_id) { 'RM6360' }
  let(:default_params) { { service: 'legal_panel_for_government/admin', framework: framework_id } }
  ...
  describe 'GET index CSV' do
    ...
    it 'download the csv' do
      expect(response.headers['Content-Disposition']).to include 'filename="Change log - Legal Panel for Government - RM6360 - '
  ...
  describe 'GET show' do
    ...
    context 'when the change type is add_rates_for_supplier_framework_lot_jurisdiction' do
      let(:change_type) { :add_rates_for_supplier_framework_lot_jurisdiction }

      include_context 'when testing a change type'
    end

    context 'when the change type is remove_rates_for_supplier_framework_lot_jurisdiction' do
