...
RSpec.describe 'supply_teachers/rm3826/suppliers/_master_vendor.html.erb' do
  ...
  before do
    ...
    render 'supply_teachers/rm3826/suppliers/master_vendor', master_vendor: supplier
  end
  ...
  context 'when there are three rates for job type' do
    let(:job_types_vs_rates) do
      {
        'qt' => [
          build(:supply_teachers_rm3826_master_vendor_rate, job_type: 'qt', term: 'one_week', mark_up: 0.11),
          build(:supply_teachers_rm3826_master_vendor_rate, job_type: 'qt', term: 'twelve_weeks', mark_up: 0.12),
          build(:supply_teachers_rm3826_master_vendor_rate, job_type: 'qt', term: 'more_than_twelve_weeks', mark_up: 0.13)
        ]
      }
    end
  ...
  context 'when there are is one rate for job type' do
    let(:job_types_vs_rates) do
      {
        'nominated' => [
          build(:supply_teachers_rm3826_master_vendor_rate, job_type: 'nominated', mark_up: 0.3)
        ]
      }
    end
    ...
    let(:job_types_vs_rates) do
      {
        ...
        'fixed_term' => [
          build(:supply_teachers_rm3826_master_vendor_rate, job_type: 'fixed_term', mark_up: 0.4)
        ]
      }
    end
