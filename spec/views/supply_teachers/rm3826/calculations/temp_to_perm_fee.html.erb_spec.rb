...
RSpec.describe 'supply_teachers/calculations/temp_to_perm_fee.html.erb', type: :view do
  ...
  context 'when hiring after 12 weeks' do
    ...
    context 'and giving enough notice' do
      ...
      it 'displays explanation' do
        expect(rendered).to have_text(I18n.t("#{i18n_key}.after_12_weeks_and_enough_notice.description"))
      end
    ...
    context 'and not giving enough notice' do
      ...
      it 'displays explanation' do
        expect(rendered).to have_text(
          I18n.t("#{i18n_key}.after_12_weeks_and_not_enough_notice.description", fee: '£50.00')
        )
      end
      ...
      it 'displays notice period required' do
        expect(rendered).to have_text(
          I18n.t("#{i18n_key}.calculations.notice_period_required", days: 20)
        )
      end

      it 'displays notice period given' do
        expect(rendered).to have_text(
          I18n.t("#{i18n_key}.calculations.notice_period_given",
                 days: 5,
      ...
      it 'displays chargeable days for lack of notice' do
        expect(rendered).to have_text(
          I18n.t("#{i18n_key}.calculations.lack_of_notice_chargeable_days", count: 15)
        )
      end

      it 'displays supplier daily fee' do
        expect(rendered).to have_text(
          I18n.t("#{i18n_key}.calculations.daily_supplier_fee", fee: '£10.00', markup_rate: '10.0%', day_rate: '£110.00')
        )
      end

      it 'displays total fee' do
        expect(rendered).to have_text(
          I18n.t("#{i18n_key}.calculations.total_fee", fee: '£150.00')
        )
      end
      ...
      context 'and the worker works fewer than 5 days a week' do
        ...
        it 'displays pro rata daily supplier fee' do
          expect(rendered).to have_text(
            I18n.t("#{i18n_key}.calculations.daily_supplier_fee_pro_rata", fee: '£4.00', days_per_week: 2)
          )
        end
      end
    end
    ...
    context 'and not entering a notice date' do
      ...
      it 'displays explanation' do
        expect(rendered).to have_text(
          I18n.t("#{i18n_key}.after_12_weeks_and_no_notice_date.description")
        )
      end
  ...
  context 'when hiring between 9 and 12 weeks' do
    ...
    context 'and giving enough notice' do
      ...
      it 'displays explanation' do
        expect(rendered).to have_text(
          I18n.t("#{i18n_key}.between_9_and_12_weeks_and_enough_notice.description")
        )
      end
      ...
      it 'displays the number of working days required' do
        expect(rendered).to have_text(
          I18n.t("#{i18n_key}.calculations.working_days_required")
        )
      end

      it 'displays the number of working days between contract start and hire date' do
        expect(rendered).to have_text(
          I18n.t("#{i18n_key}.calculations.working_days",
                 days: 50,
      ...
      it 'displays the days chargeable for early-hire' do
        expect(rendered).to have_text(
          I18n.t("#{i18n_key}.calculations.early_hire_chargeable_days", count: 10)
        )
      end
      ...
      it 'displays total fee' do
        expect(rendered).to have_text(
          I18n.t("#{i18n_key}.calculations.total_fee", fee: '£100.00')
        )
      end
    ...
    context 'and not giving enough notice' do
      ...
      it 'displays explanation' do
        expect(rendered).to have_text(
          I18n.t("#{i18n_key}.between_9_and_12_weeks_and_not_enough_notice.description")
        )
      end
      ...
      it 'displays total chargeable days' do
        expect(rendered).to have_text(
          I18n.t("#{i18n_key}.between_9_and_12_weeks_and_not_enough_notice.total_chargeable_days",
                 days: 20)
        )
      end
      ...
      it 'displays total fee' do
        expect(rendered).to have_text(
          I18n.t("#{i18n_key}.calculations.total_fee", fee: '£200.00')
        )
      end
    ...
    context 'and not entering a notice date' do
      ...
      it 'displays explanation' do
        expect(rendered).to have_text(
          I18n.t("#{i18n_key}.between_9_and_12_weeks_and_no_notice_date.description")
        )
      end
      ...
      it 'displays disclaimer about not giving enough notice' do
        expect(rendered).to have_text(
          I18n.t("#{i18n_key}.between_9_and_12_weeks_and_no_notice_date.notice_period_disclaimer",
                 max_fee: '£200.00',
  ...
  context 'when hiring within the first 8 weeks' do
    ...
    it 'displays explanation' do
      expect(rendered).to have_text(
        I18n.t("#{i18n_key}.within_first_8_weeks.description")
      )
    end
    ...
    it 'displays the days chargeable for early-hire' do
      expect(rendered).to have_text(
        I18n.t("#{i18n_key}.calculations.early_hire_chargeable_days", count: 25)
      )
    end
    ...
    it 'displays total fee' do
      expect(rendered).to have_text(
        I18n.t("#{i18n_key}.calculations.total_fee", fee: '£250.00')
      )
    end
