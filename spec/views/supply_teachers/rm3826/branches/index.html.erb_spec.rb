...
RSpec.describe 'supply_teachers/rm3826/branches/index.html.erb' do
  ...
  let(:first_supplier) { build(:supply_teachers_rm3826_supplier, name: 'First Supplier') }
  let(:second_supplier) { build(:supply_teachers_rm3826_supplier, name: 'Second Supplier') }

  let(:first_branch) do
    build(:supply_teachers_rm3826_branch_search_result, name: 'First Branch', supplier_name: 'First Supplier')
  end
  let(:second_branch) do
    build(:supply_teachers_rm3826_branch_search_result, supplier_name: first_supplier.name, name: 'Second Branch')
  end
  let(:third_branch) do
    build(:supply_teachers_rm3826_branch_search_result, supplier_name: second_supplier.name, name: 'Third Branch')
  end
  ...
  context 'when displaying branches near the buyers location' do
    ...
    let(:rates) { [build(:supply_teachers_rm3826_rate, job_type: 'nominated')] }
