...
RSpec.describe ManagementConsultancy::SupplierSpreadsheetCreator do
  ...
  let(:suppliers) { ManagementConsultancy::Supplier.all.order(:name) }
  let(:lot_number) { 'MCF3.1' }
  let(:lot) { ManagementConsultancy::Lot.find_by(number: lot_number) }
  let(:services) { ManagementConsultancy::Service.all.sample(5).sort_by(&:code) }
  ...
  let(:work_book) do
    IO.write('/tmp/mc_supplier_spreadsheet.xlsx', spreadsheet_creator.build.to_stream.read)
    Roo::Excelx.new('/tmp/mc_supplier_spreadsheet.xlsx')
  end

  before do
    supplier_details.each do |supplier_detail|
      create(:management_consultancy_supplier, **supplier_detail)
    end
  end
