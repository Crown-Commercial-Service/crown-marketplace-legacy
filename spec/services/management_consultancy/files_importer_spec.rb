...
RSpec.describe ManagementConsultancy::FilesImporter do
  ...
  let(:supplier_details_file) { ManagementConsultancy::SupplierDetailsFile.new(**supplier_details_file_options) }
  let(:supplier_details_file_path) { ManagementConsultancy::SupplierDetailsFile::OUTPUT_PATH }
  let(:supplier_details_file_options) { {} }

  let(:supplier_rate_cards_file) { ManagementConsultancy::SupplierRateCardsFile.new(**supplier_rate_cards_file_options) }
  let(:supplier_rate_cards_file_path) { ManagementConsultancy::SupplierRateCardsFile::OUTPUT_PATH }
  let(:supplier_rate_cards_file_options) { {} }

  let(:supplier_service_offerings_file) { ManagementConsultancy::SupplierServiceOfferingsFile.new(**supplier_service_offerings_file_options) }
  let(:supplier_service_offerings_file_path) { ManagementConsultancy::SupplierServiceOfferingsFile::OUTPUT_PATH }
  ...
  describe 'check_files' do
    ...
    context 'when the files have the wrong headers and columns' do
      let(:supplier_details_file_options) { { headers: ManagementConsultancy::SupplierDetailsFile.sheets_with_extra_headers(['MCF3']) } }
      let(:supplier_rate_cards_file_options) { { headers: ManagementConsultancy::SupplierRateCardsFile.sheets_with_extra_headers(['MCF3 Lot 3', 'MCF3 Lot 4']) } }
      let(:supplier_service_offerings_file_options) { { headers: ManagementConsultancy::SupplierServiceOfferingsFile.headers_with_extra_service(['MCF3 Lot 2', 'MCF3 Lot 4', 'MCF3 Lot 8']) } }
  ...
  describe 'import_data' do
    ...
    it 'publishes the data and all the suppliers are imported' do
      ...
      expect(ManagementConsultancy::Supplier.count).to eq 3
    end
