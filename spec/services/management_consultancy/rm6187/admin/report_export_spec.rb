...
RSpec.describe ManagementConsultancy::RM6187::Admin::ReportExport do
  let(:framework) { Framework.find('RM6187') }
  let(:other_framework) { create(:framework) }
  let(:user_1) { create(:user) }
  let(:user_2) { create(:user) }
  let(:user_3) { create(:user) }
  let(:search_1) { create(:search, user: user_1, framework: framework, created_at: 6.days.ago, search_criteria: { 'lot_id' => 'RM6187.1', 'service_ids' => ['RM6187.1.1', 'RM6187.1.2'] }) }
  let(:search_2) { create(:search, user: user_2, framework: framework, created_at: 5.days.ago, search_criteria: { 'lot_id' => 'RM6187.2', 'service_ids' => ['RM6187.2.1', 'RM6187.2.2'] }) }
  let(:search_3) { create(:search, user: user_3, framework: framework, created_at: 4.days.ago, search_criteria: { 'lot_id' => 'RM6187.3', 'service_ids' => ['RM6187.3.1', 'RM6187.3.2'] }) }
  let(:search_4) { create(:search, user: user_1, framework: framework, created_at: 3.days.ago, search_criteria: { 'lot_id' => 'RM6187.4', 'service_ids' => ['RM6187.4.1', 'RM6187.4.2'] }) }
  let(:search_5) { create(:search, user: user_2, framework: framework, created_at: 2.days.ago, search_criteria: { 'lot_id' => 'RM6187.5', 'service_ids' => ['RM6187.5.1', 'RM6187.5.2'] }) }
  let(:search_6) { create(:search, user: user_3, framework: framework, created_at: 1.day.ago, search_criteria: { 'lot_id' => 'RM6187.6', 'service_ids' => ['RM6187.6.1', 'RM6187.6.2'] }) }
  let(:search_7) { create(:search, user: user_1, framework: framework, created_at: Time.now.in_time_zone('London'), search_criteria: { 'lot_id' => 'RM6187.7', 'service_ids' => ['RM6187.7.1', 'RM6187.7.2'] }) }
  let(:search_8) { create(:search, user: user_1, framework: other_framework, created_at: 3.days.ago, search_criteria: { 'lot_id' => 'RM6187.4', 'service_ids' => ['RM6187.4.1', 'RM6187.4.2'] }) }
  ...
  describe '.create_headers_row' do
    ...
    it 'returns the expected headers' do
      expect(result).to eq(['User ID', 'Search date', 'Lot', 'Services', 'Suppliers'])
    end
  end
