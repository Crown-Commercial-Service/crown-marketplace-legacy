...
RSpec.describe LegalServices::SupplierSpreadsheetCreator do
  ...
  let(:suppliers) { LegalServices::Supplier.all.order(:name) }
  let(:lot_number) { '1' }
  let(:lot) { LegalServices::Lot.find_by(number: lot_number) }
  let(:services) { LegalServices::Service.all.sample(5).sort_by(&:code) }
  ...
  let(:work_book) do
    IO.write('/tmp/ls_supplier_spreadsheet.xlsx', spreadsheet_creator.build.to_stream.read)
  ...
  before do
    ...
    supplier_details.each do |supplier_detail|
      create(:legal_services_supplier, **supplier_detail)
    end
  end
