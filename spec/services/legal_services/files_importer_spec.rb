...
RSpec.describe LegalServices::FilesImporter do
  let(:upload) do
    create(:legal_services_admin_upload, aasm_state: 'in_progress') do |admin_upload|
  ...
  let(:supplier_details_file) { LegalServices::SupplierDetailsFile.new(**supplier_details_file_options) }
  let(:supplier_details_file_path) { LegalServices::SupplierDetailsFile::OUTPUT_PATH }
  let(:supplier_details_file_options) { {} }

  let(:supplier_rate_cards_file) { LegalServices::SupplierRateCardsFile.new(**supplier_rate_cards_file_options) }
  let(:supplier_rate_cards_file_path) { LegalServices::SupplierRateCardsFile::OUTPUT_PATH }
  let(:supplier_rate_cards_file_options) { {} }

  let(:supplier_lot_1_service_offerings_file) { LegalServices::SupplierLot1File.new(**supplier_lot_1_service_offerings_file_options) }
  let(:supplier_lot_1_service_offerings_file_path) { LegalServices::SupplierLot1File::OUTPUT_PATH }
  let(:supplier_lot_1_service_offerings_file_options) { {} }

  let(:supplier_lot_2_service_offerings_file) { LegalServices::SupplierLot2File.new(**supplier_lot_2_service_offerings_file_options) }
  let(:supplier_lot_2_service_offerings_file_path) { LegalServices::SupplierLot2File::OUTPUT_PATH }
  let(:supplier_lot_2_service_offerings_file_options) { {} }

  let(:supplier_lot_3_service_offerings_file) { LegalServices::SupplierLot3File.new(**supplier_lot_3_service_offerings_file_options) }
  let(:supplier_lot_3_service_offerings_file_path) { LegalServices::SupplierLot3File::OUTPUT_PATH }
  let(:supplier_lot_3_service_offerings_file_options) { {} }

  let(:supplier_lot_4_service_offerings_file) { LegalServices::SupplierLot4File.new(**supplier_lot_4_service_offerings_file_options) }
  let(:supplier_lot_4_service_offerings_file_path) { LegalServices::SupplierLot4File::OUTPUT_PATH }
  let(:supplier_lot_4_service_offerings_file_options) { {} }

  let(:files_importer) { described_class.new(upload) }

  before do
    LegalServices::Admin::Upload::ATTRIBUTES.each do |file|
      send(file).build
      send(file).write
    end
  ...
  describe 'check_files' do
    ...
    context 'when the files have the wrong headers and columns' do
      let(:supplier_details_file_options) { { headers: LegalServices::SupplierDetailsFile.sheets_with_extra_headers(['All Suppliers']) } }
      let(:supplier_rate_cards_file_options) { { headers: LegalServices::SupplierRateCardsFile.sheets_with_extra_headers(['Lot 1', 'Lot 2b - Scotland', 'Lot 4']) } }
      let(:supplier_lot_1_service_offerings_file_options) { { headers: LegalServices::SupplierLot1File.sheets_with_extra_headers(['North West England (NUTS D)', 'Yorkshire & Humberside (NUTS E)', 'East Midlands (NUTS F)']) } }
      let(:supplier_lot_2_service_offerings_file_options) { { headers: LegalServices::SupplierLot2File.sheets_with_extra_headers(['Lot 2c - Northern Ireland']) } }
      let(:supplier_lot_3_service_offerings_file_options) { { headers: LegalServices::SupplierLot3File.sheets_with_extra_headers(['All regions']) } }
      let(:supplier_lot_4_service_offerings_file_options) { { headers: LegalServices::SupplierLot4File.sheets_with_extra_headers(['All regions']) } }
