...
module LegalServices::RM3788::Admin
  RSpec.describe FilesImporter do
    ...
    describe 'check_files' do
      ...
      context 'when the files have the wrong headers and columns' do
        ...
        let(:supplier_rate_cards_file_options) { { headers: SupplierRateCardsFile.sheets_with_extra_headers(['Lot 1', 'Lot 2b - Scotland', 'Lot 4']) } }
        let(:supplier_lot_1_service_offerings_file_options) { { headers: SupplierLot1File.sheets_with_extra_headers(['North West England (NUTS D)', 'Yorkshire & Humberside (NUTS E)', 'East Midlands (NUTS F)']) } }
        ...
        let(:supplier_lot_4_service_offerings_file_options) { { headers: SupplierLot4File.sheets_with_extra_headers(['All regions']) } }
    ...
    describe 'check_processed_data' do
      context 'when a supplier has no lots' do
        let(:supplier_lot_4_service_offerings_file_options) { { supplier_duns: { 'MALOS LTD': '987654321' } } }

        it 'changes the state to failed and has the correct errors' do
          expect(upload).to have_state(:failed)
          expect(upload.import_errors).to eq [{ error: 'supplier_missing_lots', details: ['MALOS LTD'] }]
        end
      end
      ...
      let(:expected_supplier_results) do
        {
          ...
          'MALOS LTD': { regional_availabilities: 0, service_offerings: 1, rate_cards: 1 }
        }
      end
