...
RSpec.describe LegalServices::RM6374::Admin::ReportExport do
  ...
  let(:search_1) { create(:search, user: user_1, framework: framework, created_at: 6.days.ago, search_criteria: { 'lot_number' => '1a', 'service_numbers' => ['1', '2'], 'jurisdiction' => 'a' }) }
  ...
  let(:search_3) { create(:search, user: user_3, framework: framework, created_at: 4.days.ago, search_criteria: { 'lot_number' => '6', 'service_numbers' => ['6', '36'] }) }
  let(:search_4) { create(:search, user: user_1, framework: framework, created_at: 3.days.ago, search_criteria: { 'lot_number' => '1b', 'service_numbers' => ['1', '2'], 'jurisdiction' => 'b' }) }
  ...
  describe '.create_headers_row' do
    ...
    it 'returns the expected headers' do
      expect(result).to eq(['User ID', 'Search date', 'Lot', 'Services', 'Jurisdiction', 'Suppliers'])
    end
  end
  ...
  describe '.create_search_row' do
    ...
    context 'when the search is for a lot with sublot' do
      let(:search) { search_1 }
