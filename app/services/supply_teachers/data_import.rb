class SupplyTeachers::DataImport
  ...
  def generate_branches
    branches = SupplyTeachers::GenerateBranches.new(@current_data)
    branches.generate_branches
    @supplier_branches = branches.supplier_branches
  end

  def generate_pricing
    @pricing = SupplyTeachers::GeneratePricing.new(@current_data)
  ...
  def generate_vendor_pricing
    vendor_pricing = SupplyTeachers::GenerateVendorPricing.new(@current_data)
  ...
  def generate_accreditation
    accreditation = SupplyTeachers::GenerateAccreditation.new(@current_data)
    accreditation.generate_accreditation
    @supplier_accreditation = accreditation.supplier_accreditation
  end

  def merge_data
    merge_data_object = SupplyTeachers::MergeData.new(@current_data, 'pricing supplier name', 'branches supplier name', @supplier_branches, @supplier_pricing)
    merge_data_object.merge_data
    data_branches_pricing = merge_data_object.output
    @errors += merge_data_object.errors

    merge_data_object = SupplyTeachers::MergeData.new(@current_data, 'vendor supplier name', 'branches supplier name', data_branches_pricing, @supplier_vendor_pricing)
    merge_data_object.merge_data
    data_branches_pricing_vendors = merge_data_object.output
    @errors += merge_data_object.errors

    merge_data_object = SupplyTeachers::MergeData.new(@current_data, 'accreditation supplier name', 'branches supplier name', data_branches_pricing_vendors, @supplier_accreditation)
    merge_data_object.merge_data
  ...
  def add_vendor_contacts
    vendor_contacts = SupplyTeachers::AddVendorContacts.new(@current_data, @data)
    vendor_contacts.add_vendor_contacts
    @data = vendor_contacts.data_with_vendors
  end

  def validate_and_geocode
    geocodeing = SupplyTeachers::ValidateAndGeocode.new(@current_data, @data)
