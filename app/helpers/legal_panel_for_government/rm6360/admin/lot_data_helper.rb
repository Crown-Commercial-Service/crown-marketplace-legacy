  ...
  def rates_form_table_headers_and_rows(rates)
    [
      [
        ...
        {
          text: tag.span(t('shared.rates_table.rm6360.categories.default'), id: aria_describedby_id),
          classes: 'govuk-!-width-one-quarter'
        }
      ],
      @lot.positions.order(:number).map do |position|
        input = create_rate_input(position, rates, ->(position) { I18n.t("shared.rates_table.rm6360.job_titles.#{position.name}") })
