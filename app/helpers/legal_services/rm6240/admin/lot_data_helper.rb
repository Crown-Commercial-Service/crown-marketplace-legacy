module LegalServices::RM6240::Admin::LotDataHelper
  ...
  def services_lot_title(supplier_lot_data_item)
    if supplier_lot_data_item[:lot][:number] == '3'
      ...
      t(
        'legal_services.rm6240.admin.lot_data.index.supplier_lot_data_summary_list.lot_name',
        jurisdiction: t("legal_services.rm6240.admin.lot_data.index.jurisdictions.#{supplier_lot_data_item[:lot][:number][1]}"),
  ...
  def rates_form_table_headers_and_rows(rates)
    [
      [
        ...
        {
          text: tag.span(t('shared.rates_table.rm6240.categories.default'), id: aria_describedby_id),
          classes: 'govuk-!-width-one-quarter'
        }
      ],
      @lot.positions.order(:number).map do |position|
        input = create_rate_input(position, rates, ->(position) { I18n.t("shared.rates_table.rm6240.job_titles.#{position.name}") })
